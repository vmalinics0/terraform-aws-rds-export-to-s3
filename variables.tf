...
locals {
  snapshots_events_topic_arn = var.enabled ? var.create_snapshots_events_topic ? aws_sns_topic.rdsSnapshotsEvents[0].arn : var.snapshots_events_topic_arn : ""
  notifications_topic_arn    = var.enabled ? var.create_notifications_topic ? aws_sns_topic.exportMonitorNotifications[0].arn : var.notifications_topic_arn : ""
  kms_key_arn                = var.enabled ? var.create_customer_kms_key ? aws_kms_key.snapshotExportEncryptionKey[0].arn : var.customer_kms_key_arn : ""
